package com.example.rsentinel.config;
...
public class LocaleConfiguration {
  ...
  public WebFilter localeChangeFilter(LocaleContextResolver localeContextResolver) {
    return (exchange, chain) -> {
      ...
      if (newLocale != null) {
        localeContextResolver.setLocaleContext(exchange,
            new SimpleLocaleContext(StringUtils.parseLocaleString(newLocale)));
      }
      // Proceed in any case.
  ...
  static class AngularCookieLocaleContextResolver implements LocaleContextResolver {
    ...
    private static final String TIME_ZONE_REQUEST_ATTRIBUTE_NAME =
        AngularCookieLocaleContextResolver.class.getName() + ".TIME_ZONE";
    ...
    public void setLocaleContext(@Nonnull ServerWebExchange exchange, LocaleContext localeContext) {
      ...
      if (localeContext != null) {
        ...
        addCookie(
            ...
            QUOTE + (locale != null ? locale.toString() : "-") + (timeZone != null ? ' '
                + timeZone.getID() : "") + QUOTE
        );
      } else {
      ...
      exchange
          .getAttributes()
          .put(LOCALE_REQUEST_ATTRIBUTE_NAME, (locale != null ? locale
    ...
    private void addCookie(@Nonnull ServerHttpResponse response, String cookieValue) {
      ...
      ResponseCookie cookie = ResponseCookie.from(COOKIE_NAME, cookieValue).path(COOKIE_PATH)
          .build();
      response.addCookie(cookie);
      if (logger.isDebugEnabled()) {
        logger.debug(
            "Added cookie with name [" + COOKIE_NAME + "] and value [" + cookieValue + "]");
      }
    }

    private void removeCookie(@Nonnull ServerHttpResponse response) {
      Assert.notNull(response, "ServerHttpResponse must not be null");
      ResponseCookie cookie = ResponseCookie.from(COOKIE_NAME, "").path(COOKIE_PATH)
    ...
    private void parseLocaleCookieIfNecessary(ServerWebExchange exchange) {
      if (exchange.getAttribute(LOCALE_REQUEST_ATTRIBUTE_NAME) == null) {
        ...
        if (cookie != null) {
          ...
          locale =
              !"-".equals(localePart) ? StringUtils.parseLocaleString(localePart.replace('-', '_'))
                  : null;
